// Check what coords "BYU Provo" geocodes to in the app
...
async function main() {
  ...
  // Type BYU Provo and capture the geocoded result
  await page.type('#fromInput', 'Pleasant Grove, Utah');
  await wait(900);
  await page.type('#toInput', 'BYU Provo');
  await wait(1500);
  ...
  // Check what the app's state.to is
  const stateInfo = await page.evaluate(() => {
    const app = window.__gw;
    return {
      from: app.state.from,
      to: app.state.to,
      toCoords: app.state.to?.coords,
      toLabel: app.state.to?.label,
    };
  });
  console.log('State after geocoding:', JSON.stringify(stateInfo, null, 2));

  // Now route and check the result
